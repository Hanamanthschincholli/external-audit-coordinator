...
public class FindingService {
    ...
    @Cacheable(value = "findings", key = "#id")
    public Finding getFindingById(UUID id) {
        return findingRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Finding not found with id: " + id));
    }

    @Transactional(readOnly = true)
    public List<Finding> getFindingsByProgramId(UUID programId) {
        return findingRepository.findByAuditProgramId(programId);
    }

    @Transactional(readOnly = true)
    public Page<Finding> getAllFindings(Pageable pageable) {
        return findingRepository.findAll(pageable);
    }

    @Transactional
    @CacheEvict(value = {"findings", "findingsList", "findingsPaginated"}, allEntries = true)
    public Finding createFinding(UUID programId, UUID raisedById, Finding finding) {
        ...
        if (finding.getReferenceNumber() == null || finding.getReferenceNumber().isBlank()) {
            finding.setReferenceNumber(
                    "FIND-" + LocalDate.now().getYear() + "-" +
    ...
    @Transactional
    @CacheEvict(value = {"findings", "findingsList", "findingsPaginated"}, allEntries = true)
    public Finding updateFinding(UUID id, Finding findingDetails) {
    ...
    @Transactional
    @CacheEvict(value = {"findings", "findingsList", "findingsPaginated"}, allEntries = true)
    public Finding resolveFinding(UUID id) {
    ...
    @Transactional
    @CacheEvict(value = {"findings", "findingsList", "findingsPaginated"}, allEntries = true)
    public Finding assignOwner(UUID findingId, UUID ownerId) {
    ...
    @Transactional
    @CacheEvict(value = {"findings", "findingsList", "findingsPaginated"}, allEntries = true)
    public void deleteFinding(UUID id) {
